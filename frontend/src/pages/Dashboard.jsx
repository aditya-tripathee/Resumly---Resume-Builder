...
import { DeleteIcon, FilePenLineIcon, PencilIcon, PlusIcon, TrashIcon, UploadCloud, UploadCloudIcon, XIcon } from "lucide-react";
        ...
        {allResumes.map((resume, index) => {
          ...
          return (
            ...
            >
              ...
              <div onClick={(e)=>e.stopPropagation()} className="absolute top-1 group-hover:flex items-center hidden">
       ...
       
       {
        showCreateResume && (
          <form onSubmit={createResume} onClick={()=>setShowCreateResume(false)} action={""} className=" fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center">
            <div onClick={e=>e.stopPropagation()} className="border border-white rounded-2xl px-5 py-9 shadow-lg bg-slate-400 relative">
              <h2 className="mb-2 font-medium text-[18px]">Create a Resume</h2>
              <input onChange={(e)=>setTitle(e.target.value)} value={title}
               type="text" placeholder="Enter resume title" className="border-white w-full px-4 py-2 mb-4 focus:border-green-600 ring-green-600 border rounded-[5px]" required/>
              <button className="w-[100%] py-2  bg-indigo-600 rounded text-white hover:bg-green-500 cursor-pointer transition-all">Create Resume</button>
              <XIcon className="absolute top-10 right-4  transition-colors text-gray-900 hover:text-slate-600 cursor-pointer"
...

       {
        showUploadResume && (
           <form onSubmit={uploadResume} onClick={()=>setShowUploadResume(false)} action={""} className=" fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center">
            <div onClick={e=>e.stopPropagation()} className="border border-white px-15 py-10 rounded-2xl  shadow-lg bg-slate-400 relative">
              <h2 className="mb-2 font-medium text-[18px]">Upload Resume</h2>
              <input type="text" placeholder="Enter resume title" onChange={(e)=>setTitle(e.target.value)} value={title}
               className="border-white w-full px-4 py-2 mb-4 focus:border-green-600 ring-green-600 border rounded-[5px]" required/>
               <div className="">
                <label htmlFor="resume-input" className="block text-sm text-slate-800">
                  Select resume file 
              ...
              <button className="w-[100%] py-2  bg-indigo-600 rounded text-white hover:bg-green-500 cursor-pointer 
              transition-all">Upload Resume</button>
              <XIcon className="absolute top-10 right-4  transition-colors text-gray-900 hover:text-slate-600 cursor-pointer"
        ...
        editResumeId && (
          <form onSubmit={editTitle} onClick={()=>setEditResumeId('')} action={""} className=" fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center">
            <div onClick={e=>e.stopPropagation()} className="border border-white rounded-2xl px-5 py-9 shadow-lg bg-slate-400 relative">
              ...
              <input onChange={(e)=>setTitle(e.target.value)} value={title}
               type="text" placeholder="Enter resume title" className="border-white w-full px-4 py-2 mb-4 focus:border-green-600 ring-green-600 border rounded-[5px]" required/>
              <button className="w-[100%] py-2  bg-indigo-600 rounded text-white hover:bg-green-500 cursor-pointer transition-all">Update Resume</button>
              <XIcon className="absolute top-10 right-4  transition-colors text-gray-900 hover:text-slate-600 cursor-pointer"
